      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden print:h-auto print:overflow-visible">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 print:p-0 print:overflow-visible">
           ...
           <div className="max-w-6xl mx-auto">
              ...
              {/* SYSTEM INFO TAB - FIXED PART HERE */}
                        ...
                        <table className="w-full text-left border-collapse">
                          ...
                          <tbody className="divide-y divide-slate-800 text-slate-300 text-sm">
                            <tr className="hover:bg-slate-800/50 transition-colors">
                              ...
                              <td className="p-4">Bias & Threshold</td>
                              {/* FIXED: Using {'>'} and {'<'} inside curly braces to escape JSX parsing error */}
                              <td className="p-4 text-slate-400">
                                嚴格閾值控制：分數 {'>'}65 (Bullish) 加權多方飄移；分數 {'<'}45 (Bearish) 觸發避險訊號。
                              </td>
                            </tr>
                          </tbody>
                        </table>
              ...
              {!loading && activeTab === AnalysisTab.INDIVIDUAL && stockResult && (
                <div className="animate-fade-in space-y-6 pb-12 print:pb-0">
                  <div className="flex justify-between items-center mb-4 print:hidden">
                    ...
                    <button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors">
                      <Printer className="w-4 h-4" /> 輸出分析報告 (PDF)
                    </button>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 print:grid-cols-4">
                    ...
                    <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 shadow-lg print:bg-white print:border-slate-300 print:text-black">
                      <div className="text-slate-400 text-xs uppercase tracking-wider print:text-slate-600">Stage A Prediction</div>
                      <div className="text-xl font-bold mt-1 print:text-black">{getPredictionLabel(stockResult.stageAPrediction)}</div>
                    </div>
                    <div className="bg-slate-900 p-5 rounded-xl border border-slate-800 shadow-lg print:bg-white print:border-slate-300 print:text-black">
                      <div className="text-slate-400 text-xs uppercase tracking-wider print:text-slate-600">Stage B Win Rate</div>
