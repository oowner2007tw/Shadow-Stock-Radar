...
export const analyzeStockWithGemini = async (stockCode: string): Promise<StockAnalysisResult> => {
  const prompt = `
    ...
    【Step 4: 機構級深度分析報告 (Institutional-Grade Analysis)】
    請根據上述數據與量化結果，撰寫一份至少 800 字的繁體中文深度分析報告，填入 'analysisReport' 欄位。報告必須展現專業研究員視角，並嚴格包含以下結構：
    1. **Executive Summary (執行摘要)**：核心論點總結、市場情緒評估及當前投資吸引力概述。
    2. **Factor Deep Dive (因子深度剖析)**：針對 VIX 波動率環境、法人與散戶籌碼博弈 (Margin vs Chips)、以及產業題材的持續性進行交叉比對解讀。
    3. **Technical Levels (技術面位階)**：詳細拆解目前股價與均線體系的關係、關鍵支撐/壓力位分析，以及 K 線型態隱含的市場共識。
    4. **Actionable Strategy (操作建議與策略)**：給出具體的策略建議，包含進場佈局區間、停損防守位、預期目標空間，以及隨市場波動變化的應對方案。
