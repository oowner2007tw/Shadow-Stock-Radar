...
export const analyzeStockWithGemini = async (stockCode: string): Promise<StockAnalysisResult> => {
  const prompt = `
    ...
    5. **Margin_Impact**: **請搜尋 "台灣證券交易所 融資餘額" (Market Margin Balance)**。目標是取得「全體上市公司融資餘額 (約數千億台幣)」的**近期增減趨勢**。
    ...
    4. **Margin Impact (15%權重) -> 填入 'margin'**:
       - **定義**：大盤融資餘額 (Market Retail Leverage)。這是判斷**市場籌碼乾淨度**的逆勢指標。
       - **高分狀況 (>75)**：融資餘額**顯著減少** (清洗浮額)，或 大盤跌但融資減幅更大 (散戶認賠殺出 -> 底部訊號)。
       - **低分狀況 (<40)**：融資餘額**創高/大增** (過熱)，或 **大盤跌但融資增加** (散戶死扛/越攤越平 -> 高風險)。
       - **中性 (50-60)**：融資變動幅度小，與指數同步。
    ...
    - **VIX 逆勢解讀**: 解釋市場貪婪恐慌程度。
    - **大盤融資與散戶動向**: **重點分析「大盤融資餘額」**的變化。目前是「融資減肥(好事)」還是「融資暴增(壞事)」？這代表市場浮額清洗乾淨了嗎？
